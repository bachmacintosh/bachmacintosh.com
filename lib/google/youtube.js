...
export async function getVideosFeaturingMe () {
  const uri = `playlistItems?part=snippet&playlistId=${process.env.YOUTUBE_PLAYLIST_ID}&maxResults=50&key=${process.env.YOUTUBE_API_KEY}`;
  const response = await fetchYouTube(uri,);
  return extractYouTubeVideos(response,);
}
...
function extractYouTubeVideos (fetchResponse, ) {
