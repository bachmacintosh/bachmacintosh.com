...
export async function getAnime () {
  ...
  // Planning
  response.data.MediaListCollection.lists[1].entries.forEach((entry,) => {
    content.details.planning.push(entry,);
  },);
  
  // Completed
  response.data.MediaListCollection.lists[2].entries.forEach((entry,) => {
    content.details.completed.push(entry,);
  },);
  
  // Dropped
  response.data.MediaListCollection.lists[3].entries.forEach((entry,) => {
    content.details.dropped.push(entry,);
  },);
  
  // Paused
  response.data.MediaListCollection.lists[4].entries.forEach((entry,) => {
    content.details.paused.push(entry,);
  },);
